...
namespace Lab26_EFCore_Practice.Controllers
{
    public class HomeController : Controller
    {
        ...
        //разобраться в смысле двух этих методов
        [HttpPost]
        ...
        [HttpPost]
        public IActionResult AddEmpSeller([FromForm] string name, [FromForm] int role)
        {
            using (var context = new Lab26AutoContext())
            {
                var emp = new Employee { EmployeeName = name, RoleId = role};
                context.Add(auto);
                context.SaveChanges();
            }
            return RedirectToAction("Auto");
